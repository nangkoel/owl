...
function Header()
{
   ...
   $bi=substr($periode,6,2);
   $thi=substr($periode,0,4);
  ...
  $this->SetFont('Helvetica','B',20);
   ...
   $this->Cell(40);
   //Judul dalam bingkai
   $this->Cell(250,10,'Rekap Penerimaan TBS Internal Per Bulan unit :'.$unit,0,0,'C');
   //Ganti baris
   $this->Ln(10);
...
function FancyTable($header,$data)
{
   ...
   $thi=substr($periode,0,4);
	...
	for($i=0;$i<count($header);$i++)
     ...
     $query="select distinct DIVCODE from ".$dbname.".mstrxtbs where DIVCODE like '".$unit."%'";
    ...
    for($x=1;$x<=$numdays;$x++)
    {
     ...
     foreach($arrh as $key=>$val)
     {
		...
		$query="select sum(netto-kgpotsortasi) from ".$dbname.".mstrxtbs where DIVCODE  ='".$val."'
				and substr(date(dateout),9,2)='".$x."' and outin='0'
				and dateout like '".$thi."-".$bi."%' and productcode='40000003' order by DIVCODE";
				//echo 	$query."<br>";	
		$res=mysql_query($query);
    ...
    $this->Cell(15,7,'Kotor','TBLR',0,'R',1);
     ...
     {
        $totalunit=0;
        $query="select sum(netto) as netto from ".$dbname.".mstrxtbs where DIVCODE='".$val."'
 ...
 #total kotor kolom
    $this->Cell(15,7,'Potongan','TBLR',0,'R',1);
     ...
     {
        $totalunit=0;
        $query="select sum(kgpotsortasi) as netto from ".$dbname.".mstrxtbs where DIVCODE='".$val."'
                        and outin='0' and dateout like '".$thi."-".$bi."%' and productcode='40000003'
                        order by DIVCODE";
        //echo $query;
        $res=mysql_query($query);
        ...
        //untuk total potongan kotor
   	$this->Cell(30,7,number_format($totalsemuapot, 2, '.', ','),'TBLR',1,'R',1);  
        
//untuk total unit per kolomnormal
   $this->Cell(15,7,'Normal','TBLR',0,'R',1);
     foreach($arrh as $key=>$val)
     {
        $totalunit=0;
        $query="select sum(netto-kgpotsortasi) as netto from ".$dbname.".mstrxtbs where DIVCODE='".$val."'
                        and outin='0' and dateout like '".$thi."-".$bi."%' and productcode='40000003'
                        order by DIVCODE";
        //echo $query;
        $res=mysql_query($query);
