...
function saveHeader() {
    ...
    param+='&kondisilahan='+kondisilahan+'&comment='+comment+'&pengawas='+pengawas+'&asisten='+asisten+'&mengetahui='+mengetahui;		
        ...
        if (con.readyState == 4) {
            if (con.status == 200) {
                ...
                if (!isSaveResponse(con.responseText)) {
                    ...
                    document.getElementById('asisten').innerHTML = ar[0];
                    document.getElementById('mengetahui').innerHTML = ar[0];
                }
            } else {
                busy_off();
                error_catch(con.status);
...
function datakePdf(ev,tanggal,kodeblok)
{
    param='method=printPdf'+'&tanggal='+tanggal+'&kodeblok='+kodeblok;
    //alert(param);
    showDialog1('Print PDF',"<iframe frameborder=0 width='100%' height='100%'"+
        " src='kebun_slave_qc_pemupukan.php?"+param+"'></iframe>",'800','400',ev);
}
