...
if ($unit!=''){
    ...
    if ($tipe=='I'){
        $whrunit="select kodeorganisasi from ".$dbname.".organisasi where tipe='KEBUN' and namaorganisasi not like 'PLASMA%'";
    } else {
        $whrunit="select kodeorganisasi from ".$dbname.".organisasi where tipe='KEBUN' and namaorganisasi like 'PLASMA%'";
    }
}
...
$aresta="SELECT sum(luasareaproduktif) as luasareal FROM ".$dbname.".setup_blok
    WHERE kodeorg in (select kodeorganisasi from ".$dbname.".organisasi where tipe='BLOK' and LEFT(induk,4) in (".$whrunit.")) and statusblok ='TM'";
if($afdId!='')
{
    $aresta="SELECT sum(luasareaproduktif) as luasareal FROM ".$dbname.".setup_blok
...
$str="SELECT noakun, setahun FROM ".$dbname.".bgt_summary_biaya_vw 
    WHERE tahunbudget = '".$tahun."' and unit in (".$whrunit.") and noakun like '621%'";
...
$str="SELECT a.*, b.noakun FROM ".$dbname.".bgt_lbm_volume_kebun_vw a
    ...
    WHERE a.tahunbudget = '".$tahun."' and a.kebun in (".$whrunit.") and b.noakun like '621%'";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt_vw 
    WHERE tanggal like '".$periode."%' and kodeorg in (".$whrunit.") and noakun like '621%'
    GROUP BY noakun";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
    ...
    WHERE a.tanggal like '".$periode."%' and a.unit in (".$whrunit.") and b.noakun like '621%'
    GROUP BY b.noakun";
if($afdId!='')
{
    $str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
...
$str="SELECT noakun, sum(jumlah) as jumlah FROM ".$dbname.".keu_jurnaldt_vw 
    WHERE (tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and kodeorg in (".$whrunit.") and noakun like '621%'
    GROUP BY noakun";
$query=mysql_query($str) or die(mysql_error($conn));
while($res=mysql_fetch_assoc($query))
{
...
$str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
    LEFT JOIN ".$dbname.".setup_kegiatan b on a.kodekegiatan=b.kodekegiatan
    WHERE (a.tanggal between '".$tahun."-01-01' and LAST_DAY('".$periode."-15')) and a.unit in (".$whrunit.") and b.noakun like '621%'
    GROUP BY b.noakun";
if($afdId!='')
{
    $str="SELECT a.kodekegiatan, sum(a.hasilkerja) as volume, b.noakun FROM ".$dbname.".kebun_perawatan_dan_spk_vw a
