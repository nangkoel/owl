...
if ($param['proses']=='getPeriod'){
    $str="select periode from ".$dbname.".sdm_5periodegaji where kodeorg='".$param['kodeorg']."' and sudahproses='0'
              order by periode desc";
    $query=mysql_query($str) or die(mysql_error($conn));
    while($res=mysql_fetch_assoc($query))
    {
        $optPeriod.="<option value=".$res['periode'].">".$res['periode']."</option>";
    }
    echo $optPeriod;
    exit();
}
...
//cleanup data setup_temp_lokasitugas
...
while($barcek=mysql_fetch_object($rescek)){
    $del=deleteQuery($dbname, setup_temp_lokasitugas, "karyawanid=".$barcek->karyawanid);
    mysql_query($del);
}
...
$query1 = selectQuery($dbname,'datakaryawan','karyawanid,tipekaryawan,namakaryawan,jms,statuspajak,npwp,nik,lokasitugas,subbagian',"tipekaryawan='".$param['tipe']."' and 
        ...
        if($trig==1){
            ...
            $qcek=  mysql_query($scek) or die(mysql_error($conn));
                ...
                while($rcek=  mysql_fetch_assoc($qcek)){
                    $sGj="select karyawanid,tipekaryawan,namakaryawan,jms,statuspajak,npwp,nik,subbagian from ".$dbname.".datakaryawan where karyawanid='".$rcek['karyawanid']."'";
                    ...
                    $subbagian[$kar2['karyawanid']]=$kar2['subbagian'];
...
if($param['tipe']==3){
 ...
 #ambil jumlah hk tidak dibayar untuk KHT dan total tidak dibayar
     $trig=1;
     $strgjh = "select  count(*) as jlh,b.karyawanid,b.subbagian from ".$dbname.".sdm_hktdkdibayar_vw a left join 
              ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
    ...
    foreach($resgjh as $idx => $val){
        if($trig==1){
            $trig=2;
            #ambil karyawan yang pindah lokasi tugas
            $scek="select a.*,b.subbagian from ".$dbname.".setup_temp_lokasitugas a left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid "
                . "where a.kodeorg='".$param['kodeorg']."' and b.tipekaryawan='".$param['tipe']."'";
            $qcek=  mysql_query($scek) or die(mysql_error($conn));
                        #ambil karyawan yang pindah lokasi tugas
                        while($rcek=  mysql_fetch_assoc($qcek)){
...
#ambil semua komponen dari gajipokok khusus KHT dan Kontrak Harian===================== pembentukan penadapatan dan pengurang dari KBL
    $str1 = "select a.*,b.namakaryawan,b.subbagian from ".$dbname.".sdm_5gajipokok a left join 
              ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
               where a.tahun=".substr($tanggal1,0,4)." and b.tipekaryawan='".$param['tipe']."' and b.lokasitugas='".$param['kodeorg']."' 
               and  (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0";
    //echo $str1;
    ...
    $tjms=Array(); //kembali di aktifkan
     ...
     /*    $i="select a.periodegaji,a.karyawanid,a.idkomponen,a.jumlah,b.lokasitugas,b.tipekaryawan from ".$dbname.".sdm_pendapatanlaindt a left join"
        ...
        foreach($n as $idx => $val){
                ...
                $whrSb="karyawanid='".$val['karyawanid']."'";
                $optSbb=  makeOption($dbname, 'datakaryawan', 'karyawanid,subbagian', $whrSb);
                $readyData[] = array(
                'kodeorg'=>$param['kodeorg'],
                'subbagian'=>$optSbb[$val['karyawanid']],
                'periodegaji'=>$param['periodegaji'],
                'karyawanid'=>$val['karyawanid'],
                'idkomponen'=>$val['idkomponen'],
                'jumlah'=>$val['jumlah'],
...
$trig=1;
$query2="select a.karyawanid,sum(a.uangkelebihanjam) as lembur,subbagian from ".$dbname.".sdm_lemburdt a left join 
        ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
         where b.tipekaryawan='".$param['tipe']."' and b.lokasitugas='".$param['kodeorg']."' 
         and  (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0                 
         and ".$where2." group by a.karyawanid";
    ...
    foreach($lbrRes as $idx=>$row) {  
            if($trig==1){
                ...
                $qcek=  mysql_query($scek) or die(mysql_error($conn));
                while($rcek=  mysql_fetch_assoc($qcek)){
                    $queryL="select a.karyawanid,sum(a.uangkelebihanjam) as lembur,subbagian from ".$dbname.".sdm_lemburdt a left join 
                            ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
...
#query sebelumm di ubah jamhari
...
#4. Get Potongan Data============================================================
    ...
    $query3="select a.nik as karyawanid,sum(jumlahpotongan) as potongan,tipepotongan,subbagian from ".$dbname.".sdm_potongandt a left join 
              ".$dbname.".datakaryawan b on a.nik=b.karyawanid
               where b.tipekaryawan='".$param['tipe']."' and b.lokasitugas='".$param['kodeorg']."' 
               and  (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0              
               and ".$where3." group by a.nik,a.tipepotongan";
...
#5. Get Angsuran Data==========================================================
    ...
    $query4="select a.karyawanid,a.bulanan,a.jenis,subbagian from ".$dbname.".sdm_angsuran a left join 
              ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
               where b.tipekaryawan='".$param['tipe']."' and b.lokasitugas='".$param['kodeorg']."' 
               and  (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0
               and a.active=1                 
...
//echo"</pre>";         
    ...
    if($param['tipe']==3){
         $query6="select sum(a.upahkerja) as upahkerja,a.karyawanid,sum(a.upahpremi) as premi,sum(a.rupiahpenalty) as penalty,b.tipekaryawan,b.subbagian 
               from ".$dbname.".kebun_prestasi_vw a left join 
              ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
               where b.tipekaryawan='".$param['tipe']."' and b.lokasitugas='".$param['kodeorg']."' 
               and  (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0
        ...
        if($param['tipe']==3){
            ...
            */
            if ($_SESSION['empl']['regional']=='SULAWESI'){
                $query7="select sum(a.upah) as upah,a.idkaryawan as karyawanid,sum(a.premi) as premi,sum(a.penalty) as penalty,b.tipekaryawan,b.lokasitugas,b.subbagian,a.posisi  
                   from ".$dbname.".vhc_runhk_vw a left join 
                  ".$dbname.".datakaryawan b on a.idkaryawan=b.karyawanid
                ...
                $query7="select sum(a.upah) as upah,a.idkaryawan as karyawanid,sum(a.premi+a.premiluarjam) as premi,sum(a.penalty) as penalty,b.tipekaryawan,b.lokasitugas,b.subbagian,a.posisi  
                   from ".$dbname.".vhc_runhk_vw a left join 
                  ".$dbname.".datakaryawan b on a.idkaryawan=b.karyawanid
                   where b.tipekaryawan='".$param['tipe']."' and b.lokasitugas in (select kodeunit from 
                    ".$dbname.".bgt_regional_assignment where regional='".$_SESSION['empl']['regional']."') 
       ...
       // exit("Error:$query7");
        ...
        foreach($premRes2 as $idx => $val)
        {
            $scek="select kodeorg from ".$dbname.".setup_temp_lokasitugas where karyawanid='".$val['karyawanid']."'";
              ...
              $sbbagian[$val['karyawanid']]=$val['subbagian'];
...
#6.3.4 Get Premi Kemandoran
        /* $query8="select sum(a.premiinput) as premi,a.karyawanid
              ...
              ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
               ...
               and a.posting=1                   
               group by a.karyawanid"; */
			   $query8="select sum(a.premiinput) as premi,a.karyawanid,b.subbagian
               from ".$dbname.".kebun_premikemandoran a left join 
              ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
               where b.tipekaryawan='".$param['tipe']."' and b.lokasitugas='".$param['kodeorg']."' 
               and  (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0
...
//               and a.tipekaryawan=4";
       ...
       foreach($gapokbhl as $key=>$val){
             if($val>0) {
                 $whrSbb="karyawanid='".$key."'";
                 $optSbb=  makeOption($dbname, 'datakaryawan', 'karyawanid,subbagian', $whrSbb);
                 $readyData[] = array(
                    'kodeorg'=>$param['kodeorg'],
                    'subbagian'=>$optSbb[$key],
                    'periodegaji'=>$param['periodegaji'],
                    'karyawanid'=>$key,
       ...
       if($param['tipe']==3){
            $stkh="select a.karyawanid,sum(a.premi) as premi,b.tipekaryawan,b.subbagian from ".$dbname.".sdm_absensidt a 
                   left join  ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
                   where b.tipekaryawan='".$param['tipe']."'  and b.lokasitugas='".$param['kodeorg']."' 
                   and  (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0
                   and a.kodeorg like '".$param['kodeorg']."%'   
           ...
           $stkh="select a.karyawanid,sum(a.premi) as premi,b.tipekaryawan,b.subbagian from ".$dbname.".sdm_absensidt a 
            ...
            while($barky=mysql_fetch_object($reskh)){
                ...
                $sbbagian[$barky->karyawanid]=$barky->subbagian;
        ...
        #premi pemanen yang dihitung bulanan==========================================
            $stkh1="select a.karyawanid,a.rupiahpremi,b.subbagian  from ".$dbname.".kebun_premipanen a 
                left join 
              ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
                          where b.tipekaryawan='".$param['tipe']."'  and b.lokasitugas='".$param['kodeorg']."' 
               and  (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0
       ...
       foreach($premi as $idx=>$row) { //indra
         ...
         foreach($penalty as $idx=>$row) { 
           ...
           #penalty kehadiran dari absensi
             if($param['tipe']==3){
                $stkh="select a.karyawanid,sum(a.penaltykehadiran) as penaltykehadiran,b.tipekaryawan,b.subbagian from ".$dbname.".sdm_absensidt a 
                    left join 
                  ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid
                              where b.tipekaryawan='".$param['tipe']."'  and b.lokasitugas='".$param['kodeorg']."' 
                   and  (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00') and b.alokasi=0
            ...
            while($barkh=mysql_fetch_object($reskh)){
                ...
                if($rcek['kodeorg']!=''){
                  ...
                  $sbbagian[$barkh->karyawanid]=$barkh->subbagian;
            }
  ...
  #ambil penghasilan yang kena PPH per karyawan dan masuk ke variable penghasilan=================dan jamsostek juga
       foreach($id as $key=>$val){
           ...
           foreach($readyData as $dat=>$bar){
            ...
            #add to ready data================================================
             if($row>0) {
                 $whrSbb="karyawanid='".$idx."'";
                 $optSbb=  makeOption($dbname, 'datakaryawan', 'karyawanid,subbagian', $whrSbb);
                 $readyData[] = array(
                    'kodeorg'=>$param['kodeorg'],
                    'subbagian'=>$optSbb[$idx],
                    'periodegaji'=>$param['periodegaji'],
                    'karyawanid'=>$idx,
                    'idkomponen'=>$komthr,
                    'jumlah'=>$row,
           ...
           if($_SESSION['empl']['regional']=='SULAWESI'){
           #tambahan jamhari utk perhitungan pph berdasarkan excel pak firman
           $arPtkp[$param['periodegaji']]=intval(substr($param['periodegaji'],5,2));
           #akumulasi upah per orang
           $sGjBr="select sum(jumlah) as gaji,a.karyawanid,right(periodegaji,2) as bln,periodegaji from ".$dbname.".sdm_gaji a  "
                  . " left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid"
                  . " where b.lokasitugas in (select kodeunit from ".$dbname.".bgt_regional_assignment where regional='".$_SESSION['empl']['regional']."') "
                  . " and periodegaji<'".$param['periodegaji']."' and left(periodegaji,4)='".substr($param['periodegaji'],0,4)."'  and tipekaryawan='".$param['tipe']."' and (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00')"
                  . " and idkomponen in (select id from ".$dbname.".sdm_5komponenpph where regional='".$_SESSION['empl']['regional']."' and status=1)"
                  . " group by a.karyawanid";
            $qGjBr=  mysql_query($sGjBr) or die(mysql_error($conn));
            while($rGjBjr=  mysql_fetch_assoc($qGjBr)){
                      if(($rGjBjr['idkomponen']=='26')||($rGjBjr['idkomponen']=='71')||($rGjBjr['idkomponen']=='28')){
                                      $gajiSthnThrBns[$rGjBjr['karyawanid']]+=$rGjBjr['gaji'];
                              }else{
                                      $brutopph[$rGjBjr['karyawanid']]+=$rGjBjr['gaji'];
                      }
            }
            #akumulasi pph per orang
            $sAkmPPh="select sum(jumlah) as gaji,a.karyawanid,right(periodegaji,2) as bln,periodegaji from ".$dbname.".sdm_gaji a "
                . " left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid"
                . " where kodeorg in (select kodeunit from ".$dbname.".bgt_regional_assignment where regional='".$_SESSION['empl']['regional']."')"
                . " and periodegaji<'".$param['periodegaji']."' and left(periodegaji,4)='".substr($param['periodegaji'],0,4)."' "
                . " and idkomponen='44' and tipekaryawan='".$param['tipe']."'  and (b.tanggalkeluar>='".$tanggal1."' or b.tanggalkeluar='0000-00-00')"
                . " group by a.karyawanid";
            //exit("error:".$sAkmPPh);
            $qAkmPPh=  mysql_query($sAkmPPh) or die(mysql_error($conn));
            while($rAkmPPh=  mysql_fetch_assoc($qAkmPPh)){
               $akmPph[$rAkmPPh['karyawanid']]=$rAkmPPh['gaji'];
            }
          }
      ...
      $jht=$djht['persen'];	
       ...
       if($_SESSION['empl']['regional']=='SULAWESI'){    
        ...
        foreach($brutopph as $xid =>$jlh){
            ...
            $jkkperkar=$jkk/100*round($jlh);
            $jhtperkar=$jht/100*round($jlh);
            //$penghasilanSetahun[$xid]=($jlh*12)+($jkk/100*$jlh*12);
            @$penghasilanSetahun[$xid]=(round($jlh)+$gajiSthnThrBns[$xid]+(round($jlh)/$arPtkp[$param['periodegaji']])*(12-$arPtkp[$param['periodegaji']])+round($jkkperkar)+(round($jkkperkar/$arPtkp[$param['periodegaji']]))*(12-$arPtkp[$param['periodegaji']]));
            #periksa biaya jabatan=========================== 
            $biayaJab[$xid]=round($penghasilanSetahun[$xid]*$jabPersen);
            if($biayaJab[$xid]>$jabMax){#jika lebih dari max maka dibatasi sebesar max
                $biayaJab[$xid]=round($jabMax);
            }
            //$penghasilanKurangJab[$xid]=$penghasilanSetahun[$xid]-$biayaJab[$xid]-($jht/100*$jlh*12);
            @$penghasilanKurangJab[$xid]=($penghasilanSetahun[$xid]-(round($jhtperkar)+(round($jhtperkar/$arPtkp[$param['periodegaji']]))*(12-$arPtkp[$param['periodegaji']])))-round($biayaJab[$xid]);
            ...
            if($penghasilanKurangJab[$xid]<$ptkp[str_replace("K","",$statuspajak[$xid])]){
                $penghasilanKurangJab[$xid]=0;
            }else{
                $penghasilanKurangJab[$xid]=(($penghasilanKurangJab[$xid]-$ptkp[str_replace("K","",$statuspajak[$xid])])/1000)*1000;
            }
            $pkp[$xid]=$penghasilanKurangJab[$xid];//$satatuspajak ambil dr array pertama di atas    
            $zz=0;
             $sisazz=0;
             if($pkp[$xid]>0){     
                 if($pkp[$xid]<($pphtarif[0]+1)){
                     $zz=$pkp[$xid]*$pphpercent[0];
                 }
                 else if($pkp[$xid]<($pphtarif[1]+1)){
                     //50000000*0.05+(N17-50000000)*0.15
                     $zz=$pphtarif[0]*$pphpercent[0]+($pkp[$xid]-$pphtarif[0])*$pphpercent[1];
                     
                 }else if($pkp[$xid]<($pphtarif[2]+1)){
                     //50000000*0.05+200000000*0.15+(N6-250000000)*0.25
                     $zz=$pphtarif[0]*$pphpercent[0]+($pphtarif[1]-$pphtarif[0])*$pphpercent[1]+($pkp[$xid]-$pphtarif[1])*$pphpercent[2];
                 }else{
                     //50000000*0.05+200000000*0.15+250000000*0.25+(N6-500000000)*0.3
                     $zz=$pphtarif[0]*$pphpercent[0]+($pphtarif[1]-$pphtarif[0])*$pphpercent[1]+$pphtarif[1]*$pphpercent[2]+($pkp[$xid]-$pphtarif[2])*$pphpercent[3];
                 }
             /*#tahap 1: 
                ...
                #zz adalah PPh Setahun per karyawan
                 $pphSetahun[$xid]=(round($zz)*($arPtkp[$param['periodegaji']]/12))-round($akmPph[$xid]);
	   ...
	   if(!empty($pphSetahun)){
			...
			#add to ready data================================================
               $whrSbb="karyawanid='".$idx."'";
               $optSbb=  makeOption($dbname, 'datakaryawan', 'karyawanid,subbagian', $whrSbb);
               $optTgs=  makeOption($dbname, 'datakaryawan', 'karyawanid,lokasitugas', $whrSbb);
...
switch($proses) {
    ...
    case 'post':
        ...
        //exit("error".$sdel);
        if(mysql_query($sdel)){// or die(mysql_error());
			foreach($readyData as $row) {
				if($row['jumlah']==0 or $row['jumlah']=='' or $row['idkomponen']=='28' or $row['idkomponen']=='71')
				{
					...
					$optTgs=  makeOption($dbname, 'datakaryawan', 'karyawanid,lokasitugas', $rt);
			   if(($optTipe[$row['karyawanid']]==$param['tipe'])&&($optTgs[$row['karyawanid']]==$param['kodeorg'])){
						 $queryIns = insertQuery($dbname,'sdm_gaji',$row);
                                    if ($row['karyawanid']=='0000001974'){
                                        echo $queryIns;
                                    }
						 //exit("error:".$queryIns);
			...
			echo "DB Delete Error :".mysql_error()."\n".$sdel;
		}
        break;
    default:
        break;
