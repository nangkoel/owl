...
//   $prodname=$bar2[0];
  ...
  $this->SetFont('Helvetica','B',20);
   ...
   $this->Cell(40);
   //Judul dalam bingkai
   $this->Cell(250,10,'Monitoring Penerimaan TBS Eksternal Per Jam',0,0,'C');
   //Ganti baris
   $this->Ln(10);
...
*/
     $query="select distinct trpcode from wbridge.mstrxtbs where unitcode is null and trpcode like '10%'
              and millcode='".$mill."' and outin='0' and dateout like '".$tgl."%' order by trpcode";
	 //echo $query;		  
     $hasil=mysql_query($query);
     while($res2=mysql_fetch_array($hasil))
     {
     $query2="select distinct trpname from wbridge.msvendortrp where trpcode='".$res2[0]."' order by trpname";
	 //echo $query2;
    ...
    for($x=0;$x<=23;$x++)
    {
      ...
      $totalvendor=0;
      //untuk kolom Jam
      $jam=substr($array_bawah[$x],0,2).' - '.substr($array_atas[$x],0,2);
   	  $this->Cell(15,7,$jam,'TBLR',0,'R');

   	  //untuk kolom2 per unit
      /*
$query="select distinct unitcode from wbridge.mstrxtbs where length(unitcode)=4  and millcode='".$mill."' and outin='0' and date(datein)='".$tgl."'
             order by unitcode";
      $hasil=mysql_query($query);
      while ($res2=mysql_fetch_array($hasil))
      {
        $query="select sum(netto) from wbridge.mstrxtbs where unitcode='".$res2[0]."' and hour(datein)>='".$array_bawah[$x]."'
                and hour(datein)<'".$array_atas[$x]."'  and millcode='".$mill."' and outin='0' and date(datein)='".$tgl."'
                order by unitcode";
        $res=mysql_query($query);
...
*/
      ...
      while($res2=mysql_fetch_array($hasil))
      {
        $query="select sum(netto) from wbridge.mstrxtbs where trpcode='".$res2[0]."' and right(dateout,8)>'".$array_bawah[$x]."' and right(dateout,8)<='".$array_atas[$x]."'
                 and millcode='".$mill."' and outin='0' and dateout like '".$tgl."%' and productcode='40000003' order by TRPCODE";
        $res=mysql_query($query);
	  ...
	  $totalbaris=$totalvendor;
  	  ...
  	  $this->Cell(20,7,number_format($totalbaris, 2, '.', ','),'TBLR',0,'R');
      $this->Ln();
    }
...
$query="select distinct unitcode from wbridge.mstrxtbs where length(unitcode)=4
             and millcode='".$mill."' and outin='0' and date(datein)='".$tgl."' order by unitcode";
    $hasil=mysql_query($query);
    while ($res2=mysql_fetch_array($hasil))
    {
        $totalunit=0;
        $query="select sum(netto) from wbridge.mstrxtbs where unitcode='".$res2[0]."'
                and hour(datein)>='00:00:00' and hour(datein)<'23:59:59' and millcode='".$mill."' and outin='0' and date(datein)='".$tgl."'
                order by unitcode";
        $res=mysql_query($query);
...
*/
	  ...
	  $this->Cell(15,7,'Total','TBLR',0,'R',1);
    $query="select distinct trpcode from wbridge.mstrxtbs where unitcode is null and trpcode like '10%'
            and datein like '".$tgl."%' order by trpcode";
    $hasil=mysql_query($query);
    while($res2=mysql_fetch_array($hasil))
    {
      $query="select sum(netto) from wbridge.mstrxtbs where trpcode='".$res2[0]."' and right(dateout,8)>'00:00:00' and right(dateout,8)<='23:59:59'
              and millcode='".$mill."' and outin='0' and dateout like '".$tgl."%' and productcode='40000003' order by trpcode";
      //echo $query;
	  $res=mysql_query($query);
      while ($bar1=mysql_fetch_array($res))
	  
      {
        $totalvendor=$bar1[0];
		//echo $totalvendor;
        	$this->Cell(20,7,number_format($totalvendor, 2, '.', ','),'TBLR',0,'R',1);
      }
    }
	
    //untuk total keseluruhan
   	$this->Cell(20,7,number_format($totalsemua, 2, '.', ','),'TBLR',0,'R',1);
    $this->Ln();
   }
}
}
