...
function Header()
{
   ...
   $str="select * from wbridge.mstrxtbs where TICKETNO='".$TICKETNO."' and OUTIN='0'";
  // echo $query;
   $res=mysql_query($str);
   while ($bar=mysql_fetch_object($res)){;
   		//$trpname=$bar[0];
		$VEHNO=$bar->VEHNOCODE;$THNTNM=$bar->TAHUNTANAM;$TICKET=$bar->TICKETNO;
		$CTRNO=$bar->CTRNO;$SIPBNO=$bar->SIPBNO;
...
if ($SLOCBULKING=='BLK1')
  ...
  $this->SetFont('times','B',20);
   ...
   $this->Cell(14.5,10,$TICKETNO,'',0,'R');
   //Ganti baris
   $this->Ln(5);
...
*/
   //Ganti baris
   $this->Ln(10);
   $this->Cell(22,10,'No. Kendaraan : ',0,0,'R');
