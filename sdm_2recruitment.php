...
$sprd="select distinct left(tanggal,7) as periode from ".$dbname.".sdm_permintaansdm 
       where stpersetujuanhrd=1 order by tanggal desc";
$qprd=mysql_query($sprd) or die(mysql_error($conn));
while($rprd=mysql_fetch_assoc($qprd)){
...
<legend><b>Laporan Rekruitment</b></legend>
...
<tr><td colspan="2">
        <button onclick="zPreview('sdm_slave_2recruitment','<?php echo $arr?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button>
        <button onclick="zPdf('sdm_slave_2recruitment','<?php echo $arr?>','printContainer')" class="mybutton">PDF</button>
        <button onclick="zExcel(event,'sdm_slave_2recruitment.php','<?php echo $arr?>')" class="mybutton">Excel</button>
       </td></tr>
