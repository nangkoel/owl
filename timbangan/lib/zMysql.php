 ...
 */
 function selectQuery($dbname,$table,$column="*",$where="",$sort="",$distinct=false) {
	# Select
	$query = "select ";
 ...
 */
  ...
  */
 function getHolding($dbname,$org) {
       $tipe = null;
       $tmpOrg = $org;
       while($tipe!='HOLDING') {
  ...
  * O : array of org
  */
 function getOrgBelow($dbname,$org,$self=true) {
       $contOrg = array();
       $data = 'x';
       $tmpOrg = array($org);
       while(!empty($tmpOrg)) {
       ...
       if($self==true) {
	      ...
	      $data = fetchData($query);
	      $resOrg = array($data[0]['kodeorganisasi']=>$data[0]['namaorganisasi']);
       } else {
	      $resOrg = array();
       }
       foreach($contOrg as $row1) {
