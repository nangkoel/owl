...
function FancyTable($header,$data)
{
    ...
    $this->Cell(22,7,'EXTERNAL',1,0,'C',1);			 
    ...
    for($x=0;$x<=23;$x++)
    {
      ...
      $query="select distinct unitcode from ".$dbname.".mstrxtbs where length(unitcode)=4  and outin=0 and dateout like '".$tgl."%'
             order by unitcode";
      $hasil=mysql_query($query);
      while ($res2=mysql_fetch_array($hasil))
      {
        $query="select sum(netto) from ".$dbname.".mstrxtbs where unitcode='".$res2[0]."' and right(dateout,8)>'".$array_bawah[$x]."'
		...
		//untuk tbs external
        $query="select sum(netto) from ".$dbname.".mstrxtbs where (unitcode='' or unitcode is null) and right(dateout,8)>'".$array_bawah[$x]."'
                and right(dateout,8)<='".$array_atas[$x]."' and outin=0 and dateout like '".$tgl."%'
                and productcode='40000003'";
        $res=mysql_query($query);
        while ($bar=mysql_fetch_array($res))
        {
          $totalunit+=$bar[0];
		...
		$this->Cell(22,7,number_format($ex, 2, '.', ','),'TBLR',0,'R');	
    ...
    {
        $totalunit=0;
        $query="select sum(netto) from ".$dbname.".mstrxtbs where unitcode='".$res2[0]."'
                and right(dateout,8)>'00:00:00' and right(dateout,8)<='23:59:59' and outin=0 and dateout like '".$tgl."%'
                and productcode='40000003' order by unitcode";
        //echo $query;
        $res=mysql_query($query);
    ...
    //total external
        $query="select sum(netto) from ".$dbname.".mstrxtbs where (unitcode='' or unitcode is null)
                and right(dateout,8)>'00:00:00' and right(dateout,8)<='23:59:59' and outin=0 and dateout like '".$tgl."%'
                and productcode='40000003' order by unitcode";
        //echo $query;
        $res=mysql_query($query);
        while ($bar2=mysql_fetch_array($res))
        {
         ...
         $this->Cell(22,7,number_format($totex, 2, '.', ','),'TBLR',0,'R',1);
