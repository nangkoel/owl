...
if($_SESSION['empl']['tipelokasitugas']=='HOLDING' or $_SESSION['empl']['tipelokasitugas']=='KANWIL')
{
	$sOrg="select namaorganisasi,kodeorganisasi from ".$dbname.".organisasi 
               where left(kodeorganisasi,4) in (select distinct kodeorganisasi from ".$dbname.".organisasi where induk='".$_SESSION['org']['kodeorganisasi']."') and tipe not in ('BLOK','STENGINE','GUDANG','GUDANGTEMP','HOLDING','PT') order by namaorganisasi asc";	
...
$arr="##periode##tPkary##kdOrg##kdBag";
$arrKry="##period##idKry";
...
<legend><b><?php echo $_SESSION['lang']['slipgajiharian'];?></b></legend>
...
<button onclick="zPreview('sdm_slave_2slipGajiKhtSil','<?php echo $arr?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button><button onclick="zPdf('sdm_slave_2slipGajiKhtSil','<?php echo $arr?>','printContainer')" class="mybutton" name="preview" id="preview">PDF</button><button onclick="zExcel(event,'sdm_slave_2slipGajiKhtSil.php','<?php echo $arr?>')" class="mybutton" name="preview" id="preview">Excel</button></td></tr>
...
<div>
<fieldset style="float: left;">
<legend><b><?php echo "Per ".$_SESSION['lang']['karyawan']." ".getNamaOrganisasi($_SESSION['empl']['lokasitugas']);?></b><?php //echo $_SESSION['lang']['']?></legend>
<table cellspacing="1" border="0" >
...
<tr><td align="right" colspan="2"><label><i>* Khusus karyawan <?php echo getNamaOrganisasi($_SESSION['empl']['lokasitugas'])?></i></label></td></tr>
<tr><td colspan="2" align="center"><button onclick="zPreview('sdm_slave_2slipGajiKhtSil','<?php echo $arrKry?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button><button onclick="zPdf('sdm_slave_2slipGajiKhtSil','<?php echo $arrKry?>','printContainer')" class="mybutton" name="preview" id="preview">PDF</button></td></tr>
</table>
</fieldset>
</div>
...
<tr><td colspan="2" align="center"><button onclick="zPreview('sdm_slave_2slipGajiKhtSil','<?php echo $arrAfd?>','printContainer')" class="mybutton" name="preview" id="preview">Preview</button><button onclick="zPdf('sdm_slave_2slipGajiKhtSil','<?php echo $arrAfd?>','printContainer')" class="mybutton" name="preview" id="preview">PDF</button><button onclick="zExcel(event,'sdm_slave_2slipGajiKhtSil.php','<?php echo $arrAfd?>')" class="mybutton" name="preview" id="preview">Excel</button></td></tr>
</table>
</fieldset>
</div>
