...
function Header()
{
   ...
   $str="select * from wbridge.mstrxtbs where TICKETNO2='".$TICKETNO."' and IDWB ='".$IDWB."' and OUTIN='0'";
  // echo $query;
   $res=mysql_query($str);
   while ($bar=mysql_fetch_object($res)){;
   		//$trpname=$bar[0];
		$VEHNO=$bar->VEHNOCODE;$THNTNM=$bar->TAHUNTANAM;$TICKET=$bar->TICKETNO;
		$CTRNO=$bar->CTRNO;$SIPBNO=$bar->SIPBNO;$PENGIRIM=$bar->PENGIRIM;$KETERANGAN=$bar->KETERANGAN;
		$TRPCODE=$bar->TRPCODE;$SPNO=$bar->SPNO;$SPNOBULKING=$bar->SPNOBULKING;
		$DRIVER=$bar->DRIVER;$NODO=$bar->NODOTRP;$DOCNO=$bar->DOCNO;$KETERANGAN->$barKETERANGAN;
	...
	$str3="select BUYERNAME from wbridge.mscontract,wbridge.msvendorbuyer where mscontract.CTRNO = '".$CTRNO."' and msvendorbuyer.BUYERCODE=mscontract.BUYERCODE";
...
$str4="select SLOCBULKING from wbridge.mstrxtbs where mstrxtbs.TICKETNO = '".$TICKET."'";
	$res4=mysql_query($str4);
	while ($bar3=mysql_fetch_object($res4)){
		$SLOCBULKING=$bar3->SLOCBULKING;
	}
...
if ($SLOCBULKING=='BLK1')
		$customername='BULKING MILL';
	else 
		if ($SLOCBULKING=='ST01')
			$customername='STORAGE 01';
		else 
			if ($SLOCBULKING=='ST02')
				$customername='STORAGE 02';
				else if ($SLOCBULKING=='ST03')
					$customername='STORAGE 03';
					else if ($SLOCBULKING=='ST04')
						$customername='STORAGE 04';
else
		$customername='BULKING SAMPIT';
  ...
  $this->SetFont('Times','B',20);
   ...
   $this->Cell(17.5,10,$TICKETNO,'',0,'R');
   //Ganti baris
   $this->Ln(5);
...
*/
   ...
   $this->Line(0,50,210,50);
	//Ganti baris
   //$this->Ln(5);
   $this->Cell(39,10,'Pengirim                     : ',0,0,'R');
   $this->Cell(50,10,$PENGIRIM,0,0,'L');
   ...
   $this->Cell(50,10,'Kg','',0,'L');
   $this->Ln(5);
   $this->Cell(39,10,'No.Dokumen              : ',0,0,'R');
   $this->Cell(50,10,$DOCNO,0,0,'L');
   $this->Line(0,92,210,92);
   $this->Ln(5);
   $this->Cell(39,10,'Keterangan                : ',0,0,'R');
   $this->Cell(50,10,$KETERANGAN,0,0,'L');
   $this->Ln(5);
   //$this->Ln(5);
   $this->Cell(39.5,10,'Weighed By                : ',0,0,'R');
   $this->Cell(50,10,$USERNAME,0,0,'L');
   $this->Cell(60.5,10,'Signature',0,0,'R');
   //$this->Ln(5);
   $this->Line(136,115,180,115);
   $this->Ln(23);
