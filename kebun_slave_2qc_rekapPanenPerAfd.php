...
$i="select distinct substr(kodeblok,1,6) as afd from ".$dbname.".kebun_qc_panendt where kodeblok like '%".$div."%' and tanggalcek like '%".$per."%' group by substr(kodeblok,1,6) order by substr(kodeblok,1,6) asc";
...
$str="	select sum(jjgpanen) as jjgpanen,sum(jjgtdkpanen) as jjgtdkpanen,sum(brdtdkdikutip)/sum(jjgpanen) as rasio
		from ".$dbname.".kebun_qc_panendt where kodeblok like '%".$div."%' and  tanggalcek like '%".$per."%' group by substr(kodeblok,1,6) order by substr(kodeblok,1,6) asc";
//exit("Error:$str");
$yPanen[$y]=0;
$yTdkPanen[$y]=0;
$yRasio[$y]=0;
...
$graph->title->Set(strtoupper($_SESSION['lang']['divisi']." ".$div." ".$nmBulan." ".$thn));
...
$str1="select sum(jjgtdkpanen) as jjgtdkpanen,sum(jjgpanen) as jjgpanen, sum(jjgtdkkumpul) as jjgtdkkumpul,
		sum(brdtdkdikutip) as brdtdkdikutip,substr(kodeblok,1,6) as afd 
		from ".$dbname.".kebun_qc_panendt where kodeblok like '%".$div."%' and tanggalcek like '%".$per."%' group by substr(kodeblok,1,6) order by substr(kodeblok,1,6) asc";
//echo $str1;		
...
while($bar1=mysql_fetch_assoc($res1))
{
	...
	$tab.="
			<tr class=rowcontent>
				<td align=left>".$bar1['afd']."</td>
				<td align=left>".$optNmOrg[$bar1['afd']]."</td>
