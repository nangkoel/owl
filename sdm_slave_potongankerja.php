...
//                        }else{
                            $ql2="select count(*) as jmlhrow from ".$dbname.".sdm_potonganht 
                                  where substring(kodeorg,1,4)='".$_SESSION['empl']['lokasitugas']."' and tipepotongan in (select distinct id  from ".$dbname.".sdm_ho_component where pinjamanid=1 order by name asc) ".$whrCr." order by `periodegaji` desc";// echo $ql2;
                            $slvhc="select * from ".$dbname.".sdm_potonganht 
                                    where substring(kodeorg,1,4)='".$_SESSION['empl']['lokasitugas']."' and tipepotongan in (select distinct id  from ".$dbname.".sdm_ho_component where pinjamanid=1 order by name asc)  ".$whrCr."
                                    order by `periodegaji` desc limit ".$offset.",".$limit."";
                        //}
                        //echo $slvhc;
