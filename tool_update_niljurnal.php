...
function loopClosingFisik(currRow,maxRow){
        notrans = document.getElementById('nojurnal_'+currRow).innerHTML;
        noref = document.getElementById('noreferensi_'+currRow).innerHTML;
        jml = document.getElementById('jmlh_'+currRow).value;
        param = "nojurnal="+notrans+'&proses=updateData';
        param+="&noreferensi="+noref+"&jmlh="+jml;
        post_response_text('kebun_slave_3updategajibjr.php', param, respon);
	...
	if(confirm('Anda Yakin Ingin Mengupdate Semua Data ..?')){
		 ...
		 for(i=1;i<=maxRow;i++){
                    try{
                            ...
                            if(strUrl2 != ''){					
                                //+'&kdbrg[]='+encodeURIComponent(trim(document.getElementById('rkdbrg_'+i).value))
                                    strUrl2 +='&nojuranl[]='+trim(document.getElementById('nojurnal_'+i).innerHTML)
                                    +'&noreferensi[]='+encodeURIComponent(trim(document.getElementById('noreferensi_'+i).innerHTML))
                                    +'&jumlah[]='+document.getElementById('jmlh_'+i).value;
                            }
                            else{
                                      strUrl2 +='&nojuranl[]='+trim(document.getElementById('nojurnal_'+i).innerHTML)
                                    +'&noreferensi[]='+encodeURIComponent(trim(document.getElementById('noreferensi_'+i).innerHTML))
        ...
        param='proses=updateData';
        param+=strUrl2;
        post_response_text('tool_slave_update_niljurnal.php', param, respon);
//        alert(param);
//        return;
	//lockScreen('wait');
	function respon(){
...
$sPrd="select distinct periode from ".$dbname.".setup_periodeakuntansi order by periode desc";
...
<legend><b>Update Jurnal Perawatan</b></legend>
...
<tr><td><label>".$_SESSION['lang']['kodeorg']."</label></td><td><select id=\"kdOrg\" name=\"kdOrg\" style=\"width:150px\">".$optOrg."</select></td></tr>
    ...
    <td><select id=\"periode\" style=\"width:150px;\" >".$optPrd."</select></td>
</tr>
<tr>
    <td><label>".$_SESSION['lang']['tipetransaksi']."</label></td>
    <td><select id=\"tpTrk\" style=\"width:150px;\" >".$optTipe."</select></td>
</tr>
<tr height=\"20\"><td colspan=\"2\">&nbsp;</td></tr>
<tr><td colspan=\"2\"><button onclick=\"zPreview('tool_slave_update_niljurnal','".$arr."','printContainer')\" class=\"mybutton\" name=\"preview\" id=\"preview\">Preview</button></td></tr>
</table>
</fieldset>
</div>
...
//========================
$hfrm[0]="Update Jurnal Perawatan";
