...
$nmKar=makeOption($dbname,'datakaryawan','karyawanid,namakaryawan',"bagian in ('PUR','LOG')");
        ...
        $periode1=$_POST['periodedari'];
        $periode2=$_POST['periodesampai'];
        ...
        switch($proses)
        {
                case'getData':
						...
						//update indra 10-02-14 sort tipe lokasitugas
                        ...
                        $offset=$page*$limit;
                        $sLim="SELECT count(*) as jmlhrow FROM ".$dbname.".log_prapodt a left join ".$dbname.".log_prapoht b on a.nopp=b.nopp where substr(b.tanggal,1,7) like '".$tglSkrng."%' ".$sortLokasi." order by a.nopp desc";
                        $query2=mysql_query($sLim) or die(mysql_error());
                        while($jsl=mysql_fetch_object($query2)){
                        $jlhbrs= $jsl->jmlhrow;
                        }
                        //$sql="select * from ".$dbname.".log_podt where nopp='".$nopp."'";
                        $sql="select a.*,b.*, a.keterangan as keterangan2 FROM ".$dbname.".log_prapodt a left join ".$dbname.".log_prapoht b on a.nopp=b.nopp where substr(b.tanggal,1,7) like '".$tglSkrng."%' ".$sortLokasi."  order by a.nopp desc limit ".$offset.",".$limit."";
                        ...
                        if($row>0)
                        {
                                ...
                                while($res=mysql_fetch_assoc($query2))
                                {
                                        ...
                                        $sDet="select nopo from ".$dbname.".log_po_vw where statuspo<>5 and nopp='".$res['nopp']."' and kodebarang='".$res['kodebarang']."'"; 
...
//			echo mysql_error($conn);
                ...
                if(($nopp=='')&&($tglSdt=='')&&($statusPP=='')&&($periode1=='')&&($periode2=='')&&($lokBeli=='')&&($_POST['supplier_id']=='')&&($_POST['txNmbrg']=='')&&($_POST['purchsrId']==''))
                {
                    $tglSkrng=date("Y-m");	
			    ...
			    //penambahan statusPP untuk 2 di query
                    ...
                    if($_POST['purchsrId']!='0000000000'){
                        $where.=" and purchaser='".$_POST['purchsrId']."'";
                    }

                    $sql="select a.*,b.* FROM ".$dbname.".log_prapodt a left join ".$dbname.".log_prapoht b on a.nopp=b.nopp  ".$where." ".$sortLokasi." order by b.tanggal desc,a.nopp desc ";
                }
                   //exit("Error".$sql);
