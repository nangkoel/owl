...
if($perod!=''){
    ...
    $where="a.sistemgaji='Harian' and a.periodegaji='".$perod."'  
            and b.subbagian='".$afdId."'";
    }else{
        $where="a.sistemgaji='Harian' and a.periodegaji='".$perod."'  
            and a.kodeorg='".substr($idAfd,0,4)."' and (b.subbagian is null or subbagian='')";
    }
    $periode=$perod;
}
...
switch($proses)
{
        ...
        $sSlip="select distinct a.*,b.tipekaryawan,b.statuspajak,b.tanggalmasuk,b.nik,b.namakaryawan,b.bagian,c.namajabatan,d.nama from 
               ...
               left join ".$dbname.".sdm_5departemen d on b.bagian=d.kode where ".$where." ".$dtTipe." and tipekaryawan=4 and idkomponen<>71";
        //exit("Error".$sSlip);
        ...
        var $dbname;
            ...
            $sAbsn="select absensi,tanggal,karyawanid from ".$dbname.".sdm_absensidt 
                            where tanggal between '".$rTgl['tanggalmulai']."' and '".$rTgl['tanggalsampai']."' and kodeorg like '".$idAfd."%' and absensi not in ('MG','L','C')";
                         //exit("Error".$sAbsn);
        ...
        #tambahan absen permintaan abis disini#
                ...
                #premi rajin
                 ...
                 $sGpk="select sum(jumlah) as jumlah,idkomponen,a.karyawanid from ".$dbname.".sdm_gaji a left join ".$dbname.".datakaryawan b on a.karyawanid=b.karyawanid "
                    . " where b.tipekaryawan=4 and idkomponen<>71 and a.kodeorg='".$idAfd."' and periodegaji='".$periode."' group by a.karyawanid,a.idkomponen";
                ...
                $arrPlusId=array("1"=>"Tarif Upah Pokok","2"=>"Hari Premi","3"=>"Gaji Pokok","4"=>"Job Insentif","5"=>"Lembur"
                           ,"6"=>"Rapel/Cuti diuangkn","7"=>"Satuan","8"=>"Tam.Insentif Satuan","9"=>"Premi Hadir","10"=>"Simpanan");
                //prepare array data gaji karyawan,nama,jabatan,tmk dan bagian
                       ...
                       while($rDptLaen=  mysql_fetch_assoc($qDptLaen)){
                        ...
                        foreach($arrKary as $lstKary){
                            ...
                            $sKehadiran="select karyawanid,sum(umr) as upah,kodekegiatan,sum(hasilkerja) as hslkrj,sum(insentif) as premikrj from ".$dbname.".kebun_kehadiran_vw 
                                         where tanggal between '".$rTgl['tanggalmulai']."' and '".$rTgl['tanggalsampai']."' and karyawanid='".$lstKary."' and tipekaryawan='KHT' and jurnal=1
                             ...
                             if($rPremiPanen['rupiahpremi']!='0'){
                                     ...
                                     if($kdData=='SULAWESI'){
                                            if($kdOrg[$optSubbgain[$rPremiPanen['karyawanid']]]!=''){
                                                if($rPremiPanen['rupiahpremi']==($rpPrem[$optSubbgain[$rPremiPanen['karyawanid'].$angk1]]*$rPremiPanen['totalkg'])){
                                                 $keg="prpanen11";
   ...
   //                                              $pkkRpSat[$rPremiPanen['karyawanid'].$tglTerakhir.$keg]=$rPremiPanen['rupiahpremi'];
                                                 ...
                                                 $komp=7;//satuan sumber dari premi panen
                                         ...
                                         if($rupy!=0){
                                             if($rpPrem[$kdData.$angk3]==(round($rPremiPanen['rupiahpremi']-$rupy))){
                                                $keg="prpanen25";
                                                $lstKeg[$rPremiPanen['karyawanid']][]=$keg;
                                                $hslRup[$rPremiPanen['karyawanid'].$keg]=($rPremiPanen['rupiahpremi']-$rupy);
                                                $hslKrj[$rPremiPanen['karyawanid'].$keg]=$rPremiPanen['totalkg'];
                                                 $komp=8;//tambahan tam insentif dari premi rajin
                                                $arrPles[$rPremiPanen['karyawanid'].$komp]+=$rupy;
                                                 $komp=7;//satuan sumber dari premi panen
                                                 $arrPles[$rPremiPanen['karyawanid'].$komp]+=($rPremiPanen['rupiahpremi']-$rupy);
                                                ...
                                                $komp=7;//satuan sumber dari premi panen
                                                 $arrPles[$rPremiPanen['karyawanid'].$komp]+=($rPremiPanen['rupiahpremi']-$rupy);
                                            }elseif((round($rPremiPanen['rupiahpremi'])-round($rupy))==0){ //$pkkRpSat[$rPremiPanen['karyawanid'].$keg]=$rPremiPanen['rupiahpremi'];
                                                $hslRup[$rPremiPanen['karyawanid'].$keg]=$rPremiPanen['rupiahpremi'];
                                                $hslKrj[$rPremiPanen['karyawanid'].$keg]=$rPremiPanen['totalkg'];
                                                $komp=8;//tambahan tam insentif dari premi rajin
                                                $arrPles[$rPremiPanen['karyawanid'].$komp]+=$rPremiPanen['rupiahpremi'];
                            ...
                            $keg=$rForm['jabatan'];
                                if($rForm['jabatan']=='LOADINGFORD'){
                                    $sKg="select sum(hasilkerja) as totalkg from ".$dbname.".kebun_kehadiran_vw "
                                        . " where karyawanid='".$rForm['karyawanid']."' and kodekegiatan in('611020228','611020224') and tanggal between '".$rTgl['tanggalmulai']."' and '".$rTgl['tanggalsampai']."' ";
                                }else{
                                    $sKg="select sum(hasilkerja) as totalkg  from ".$dbname.".kebun_kehadiran_vw "
                                        . " where karyawanid='".$rForm['karyawanid']."' and kodekegiatan in('611020221') and tanggal between '".$rTgl['tanggalmulai']."' and '".$rTgl['tanggalsampai']."' ";
                                }
                                //exit("error".$sKg);
                        ...
                        $sTksi="select sum(upah) as upah,sum(premi) as premi,idkaryawan from ".$dbname.".vhc_runhk_vw "
                                . " where statuskaryawan='KHT' and tanggal between '".$rTgl['tanggalmulai']."' and '".$rTgl['tanggalsampai']."' and kodeorg='".$idAfd."' group by idkaryawan";
                        //echo $sTksi;
...
//                            $hslRup[$rTksi['idkaryawan']."H"]+=$rTksi['upah'];
                        }
//                        $sTksi="select count(*) as kehdrn,idkaryawan from ".$dbname.".vhc_runhk_vw "
//                                . " where statuskaryawan='KHT' and tanggal between '".$rTgl['tanggalmulai']."' and '".$rTgl['tanggalsampai']."' and kodeorg='".$idAfd."' and upah!=0 group by idkaryawan";
//                        $qTksi=  mysql_query($sTksi) or die(mysql_error($conn));
//                        while($rTksi=  mysql_fetch_assoc($qTksi)){
//                            $keg="H";
//                            $hslKrj[$rTksi['idkaryawan'].$keg]+=$rTksi['kehdrn'];
//                        }
                  ...
                  foreach($dtAbsByr as $dtJmlhAbsDbyr){
                    $hrHadir[$dtKary]+=$brt[$dtKary][$dtJmlhAbsDbyr];
                               ...
                               // $pdf->Cell(99,4,"",0,1,'L');
                                ...
                                $pdf->SetFont('Arial','',8);
                                $pdf->Cell(25,4,"Hari Hadir    ",0,0,'L');
                                $pdf->Cell(3,4,":",0,0,'L');
                                $pdf->Cell(17,4,$hrHadir[$dtKary],0,0,'L');
                                ...
                                $pdf->Cell(3,4,":",0,0,'L');
                                $pdf->Cell(10,4,$dendaPnn[$dtKary],0,1,'L');
                                 ...
                                 //exit("error".$pdf->GetX());
                                    ...
                                    $pdf->SetX($xtakhir+84);
                                        ...
                                        switch(trim($keg)){
                                            ...
                                            case'prpanen25':
                                                 $optNmKeg[$keg]="INSENTIF PANEN 25 TON";
                                                 $optSatKeg[$keg]="KG";
                                            break;
                                            case'prpanen30':
                                                 $optNmKeg[$keg]="INSENTIF PANEN 30 TON";
                                                 $optSatKeg[$keg]="KG";
                                            break;
                                            case'prpanen35':
                                                $optNmKeg[$keg]="INSENTIF PANEN 35 TON";
                                                $optSatKeg[$keg]="KG";
                                            break;
                                            case'prpanen5':
                                            ...
                                            break;
                                            case'prpanen11':
                                                 $optNmKeg[$keg]="INSENTIF PANEN 11 TON";
                                                 $optSatKeg[$keg]="KG";
                                            break;
                                            case'LOADINGFORD':
                                        ...
                                        $pdf->Cell(40,4,str_replace("PERUSAHAAN", "PERSH",$optNmKeg[$keg]),0,0,'L');
        ...
        $dzstr="SELECT a.tanggal,idkaryawan FROM ".$dbname.".vhc_runhk a
        left join ".$dbname.".datakaryawan b on a.idkaryawan=b.karyawanid
        where upah!=0 and a.tanggal between '".$rTgl['tanggalmulai']."' and '".$rTgl['tanggalsampai']."' and notransaksi like '%".$idAfd."%'";
        //exit("Error".$dzstr);
        $dzres=mysql_query($dzstr);
        while($dzbar=mysql_fetch_object($dzres))
        {
        ...
        $idBln=intval($bln[1]);	
          ...
          //array data komponen penambah dan pengurang
          $sKomp="select id,name from ".$dbname.".sdm_ho_component where plus='1'  and id not in ('26','28','71') ";
          $qKomp=mysql_query($sKomp) or die(mysql_error());
          while($rKomp=mysql_fetch_assoc($qKomp))
          {
              $arrIdKompPls[]=$rKomp['id'];
         ...
         $sSlip="select distinct a.*,b.tipekaryawan,b.statuspajak,b.tanggalmasuk,b.nik,b.namakaryawan,b.bagian,c.namajabatan,d.nama,
               ...
               left join ".$dbname.".sdm_5jabatan c on b.kodejabatan=c.kodejabatan 
               left join ".$dbname.".sdm_5departemen d on b.bagian=d.kode where ".$where."  ".$dtTipe." and b.tipekaryawan=4 and idkomponen<>71";	
        // exit("Error:".$sSlip);
        $qSlip=mysql_query($sSlip) or die(mysql_error());
        $rCek=mysql_num_rows($qSlip);
        if($rCek>0)
