...
if(@$_GET['offset'])
{
	if(strlen(@$_GET['cari']))
	{
		$str="select * from ".$dbname.".msvehicle where ".$_GET['field']." like '%".$_GET['cari']."%' order by ".$order." limit ".$_GET['offset'].",10";
	}
	else
	{
	$str="select * from ".$dbname.".msvehicle order by ".$order." limit ".$_GET['offset'].",10";
	}
$offset=$_GET['offset'];
$no=$offset;
...
else
{
	if(strlen(@$_GET['cari']))
	{
		$str="select * from ".$dbname.".msvehicle where ".$_GET['field']." like '%".$_GET['cari']."%' order by ".$order." limit 0,10";
	}
else
{
$str="select * from ".$dbname.".msvehicle order by ".$order." limit 0,10";
}
$offset=0;
$no=0;
}
 ...
 {  $no+=1;
    $ter="select TRPNAME from ".$dbname.".msvendortrp where TRPCODE='".$bar->TRPCODE."' order by TRPNAME";
 	...
 	echo "<tr class=rowcontentClick>
		  ...
		  <td align=left>".$bar->VEHNOCODE."</td>
		  <td align=left>".$trpname."</td>
...
if(@$_GET['cari'])
 {
  $str1="select * from ".$dbname.".msvehicle where ".$_GET['field']." like '%".$_GET['cari']."%'";
 }
 else
 {
 $str1="select * from ".$dbname.".msvehicle";
 }
