...
$str="select sum(debet)-sum(kredit) as saldo FROM ".$dbname.".keu_jurnaldt_vw where tanggal between '".$currstart."' and '".$currend."' 
       ...
       #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Insert Header
        ...
        if($headErr=='') {
            ...
            if($detailErr=='') {
                    ...
                    createSaldoAwalKas($param['periode'],$tahunLanjut.'-'.addZero($bulanLanjut,2),$param['kodeorg']);
...
function createSaldoAwal($dariperiode,$keperiode,$kodeorg)
{
    ...
    $str="select noakun,sum(debet) as debet,sum(kredit) as kredit from ".$dbname.".keu_jurnaldt_vw 
          where tanggal between '".$currstart."' and '".$currend."' and kodeorg='".$kodeorg."' group by noakun";
    $res=mysql_query($str);
    while($bar=mysql_fetch_object($res))
    {
...
function createSaldoAwalKas($dariperiode,$keperiode,$kodeorg)
{
    global $conn;
    global $dbname;
    global $currstart;
    ...
    $str="select noakun,tipetransaksi,sum(jumlah) as jumlah from ".$dbname.".keu_kasbankht where noakun like '1110%'
          AND posting=1 AND tanggalposting between '".$currstart."' and '".$currend."' and kodeorg='".$kodeorg."' group by noakun,tipetransaksi";
    //exit('error'.$str);
    $res=mysql_query($str);
    while($bar=mysql_fetch_object($res))
    {
    ...
    if(mysql_query($str))
    {
        ...
        { 
            $salak[$key]=$mtdebet[$key]+$sawal[$key]-$mtkredit[$key];
            ...
            if(!mysql_query($temp))
            {
                exit("Error insert saldo awal kas ".mysql_error($conn).":".$temp);
            }  
        }
    }   
}   
